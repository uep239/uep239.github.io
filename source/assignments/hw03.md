...
- The specified input file does exists but the program is unable to read it for some reason.
...
```{tip}
You can use `sys.argv` to get a list of command-line arguments passed to your script. And although it is entirely possible to use `sys.argv` along with control flow statements to create a command-line interface for your program, there already exists a Python module that does that. You might want to check out the `argparse` module to create your command-line interface. Note that there also exists an alternative to `argparse` called `getopt`, which allows for an interface more familiar to C/C++ users but is otherwise not recommended.
```
